...
start_process(void *file_name_)
{
  ...
  { // Child is loaded successfully
    ...
    struct list *children = &parent->child_process;
...
void process_exit(void)
{
  ...
  if (cur->parent_thread != NULL)
  {
    ...
    {
      parent->child_status = thread_current()->thread_exiting_status; // Set parent to my exit status
      parent->waiting_on = -1;                                        // reset waiting for tid
      parent->child_creation_success = 0;                             // reset child creation success
      sema_up(&parent->wait_child);                                   // Wake up parent
    }
  }
  ...
  thread_current()->parent_thread = NULL;

  struct list *process_files = &thread_current()->open_file;
  ///// free all open files
  ...
  // Remove all children
  struct list *all_children = &thread_current()->child_process;
...
{
  struct list *children = &(thread_current()->child_process);
